...
import sys
sys.path.insert(1, os.path.join(sys.path[0], os.pardir, 'common'))
...
def is_new_frame_better(source, driving, precitor):
    ...
    new_kp = predictor.get_normalized_frame_kp(driving)
...
def change_avatar(predictor, new_avatar):
    ...
    avatar_kp = predictor.get_normalized_frame_kp(new_avatar)
    kp_source = None
    avatar = new_avatar
...
def select_camera(config):
    ...
    if os.path.isfile(cam_config):
        ...
        if cam_frames:
            ...
            logging.info(f"Selected camera {cam_id}")

            with open(cam_config, 'w') as f:
                yaml.dump({'cam_id': cam_id}, f)
        else:
            logging.info("No cameras are available")
...
if __name__ == "__main__":
    ...
    with open(client_path + '/config/config.yaml', 'r') as f:
    ...
    try:
        while True:
            ...
            if passthrough:
                ...
                if out is None:
                    logging.info('predict returned None')
            ...
            if key == 27: # ESC
                ...
                if not is_calibrated:
                    cv2.namedWindow('impersonator', cv2.WINDOW_GUI_NORMAL)
                    cv2.moveWindow('impersonator', 600, 250)
                ...
                try:
                    logging.info('Loading StyleGAN avatar...')
                    avatar = load_stylegan_avatar()
                    passthrough = False
                    change_avatar(predictor, avatar)
                except:
                    logging.info('Failed to load StyleGAN avatar')
            elif key == ord('l'):
                try:
                    logging.info('Reloading avatars...')
                    avatars = load_images()
                    passthrough = False
                    logging.info("Images reloaded")
                except:
                    logging.info('Image reload failed')
            elif key == ord('i'):
    ...
    logging.info("stopping remote predictor")
    predictor.stop()
